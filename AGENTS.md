...
> The sections below are Jarvis-specific. Keep them lean: if a rule is derivable from the code, it
> does not belong here. The **design** source of truth is [`wiki/`](./wiki/index.md) (start at
> [`wiki/status.md`](./wiki/status.md)); this file is about *how to work*, not *what we're building*.
...
- **The wiki is the design source of truth.** When the phase or the "next action" changes, update
  ...
  [`wiki/AGENTS.md`](./wiki/AGENTS.md) — follow them for wiki changes (this file is for *code*).
- Don't add ceremony (heavy ADRs, broad refactors) the project hasn't asked for — see
  [`wiki/AGENTS.md`](./wiki/AGENTS.md) on decision ceremony.
...
| `Sources/JarvisCore/Brain/` | The LLM integration: provider-neutral `BrainClient` contracts/models at the root; OpenAI and local-agent provider adapters under `Adapters/` |
