...
RUN="$TMP/live"
mkdir "$RUN"
expect_status 1 'live run with no scenario evidence fails' check_live_test_completion "$RUN" D
for scenario in A B C D R F01-system F01-microphone F02; do
  mkdir "$RUN/$scenario"
...
expect_status 0 'selected D scenario complete' check_live_test_completion "$RUN" D
expect_status 0 'F01 requires both variants' check_live_test_completion "$RUN" F01
rm "$RUN/D/results.txt"
expect_status 1 'missing selected scenario results fails' check_live_test_completion "$RUN" D
expect_status 1 'all includes D' check_live_test_completion "$RUN" all
printf '\n' > "$RUN/D/results.txt"
expect_status 1 'blank results fail' check_live_test_completion "$RUN" D
printf 'G01 pass\n' > "$RUN/D/results.txt"
rm "$RUN/D/live-e2e-finished"
expect_status 1 'missing selected scenario finish marker fails' check_live_test_completion "$RUN" D
touch "$RUN/D/live-e2e-finished"
printf 'G01 fail incomplete\n' > "$RUN/D/results.txt"
expect_status 1 'failed scenario results fail' check_live_test_completion "$RUN" D
printf 'G01 pass\n' > "$RUN/D/results.txt"
rm -r "$RUN/F01-microphone"
expect_status 1 'missing F01 variant fails' check_live_test_completion "$RUN" F01
expect_status 0 'unselected missing scenario does not fail' check_live_test_completion "$RUN" D
...
expect_status 0 'live wrapper accepts completed D' run_live D
expect_status 1 'live wrapper all requires absent F01 variant' run_live all
printf '◇ Test run started.\n' > "$SYNTHETIC_LOG"
expect_status 1 'live wrapper rejects exit zero without summary' run_live D
printf '✔ Test run with 1 test passed after 0.001 seconds.\n' > "$SYNTHETIC_LOG"
SYNTHETIC_STATUS=7
expect_status 1 'live wrapper rejects runner failure despite completion evidence' run_live D
SYNTHETIC_STATUS=0
rm "$RUN/D/live-e2e-finished"
expect_status 1 'live wrapper rejects missing finish marker' run_live D
touch "$RUN/D/live-e2e-finished"
rm "$RUN/D/results.txt"
expect_status 1 'live wrapper rejects missing results' run_live D
mkdir "$TMP/empty-live"
SYNTHETIC_LIVE_RESULTS="$TMP/empty-live"
expect_status 1 'live wrapper rejects empty run' run_live D
printf 'G01 pass\n' > "$RUN/D/results.txt"
SYNTHETIC_LIVE_RESULTS="$RUN"
export SYNTHETIC_APP_PROCESS='/workspace/Jarvis Code with AI.app/Contents/MacOS/JarvisApp'
rm -f "$SYNTHETIC_SWIFT_CALLED"
expect_status 1 'live wrapper rejects a running user preview' run_live D
if [[ -e "$SYNTHETIC_SWIFT_CALLED" ]]; then
  echo 'FAIL: live runner started tests while preview was running' >&2
  exit 1
fi
