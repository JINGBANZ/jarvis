...
|---|---|---|
| **AudioInput** | Capture the mic (`me`) with a plain `AVAudioEngine` tap (not `VoiceProcessingIO`, which silenced the mic once the `SCStream` started); stream 24 kHz PCM16 — cleaned by **EchoCanceller** — to its Transcriber. | AVFoundation (`AVAudioEngine` tap). |
| **SystemAudioInput** | Capture the other side (`them`) — system output audio; stream the same 24 kHz PCM16 to its own Transcriber, and feed it to **EchoCanceller** as the echo reference. Degrades gracefully without the Screen-Recording grant. | ScreenCaptureKit (`SCStream`, `capturesAudio`, `excludesCurrentProcessAudio`). |
| **EchoCanceller** | Remove the other side's voice that bleeds off the speakers into the mic, so it isn't transcribed a second time as `me` — no headphones required. Cleans the mic against the system audio as the far-end reference; resamples our 24 kHz wire to AEC3's 48 kHz on 10 ms frames. | WebRTC **AEC3** (`webrtc-audio-processing`), vendored static + zero-dylib via `scripts/build-aec.sh`. |
