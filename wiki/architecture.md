...
- **The OpenAI key stays required**: transcription always runs on the Realtime API. A CLI provider
  ...
  turns. The issue #111 design records the pre-removal JSON/MCP experiment: both transports used the
  same synthetic evidence, while MCP kept capture→terminal in one process. That is evidence for
  action continuity, not general coaching intelligence; any broader coaching-quality claim needs
  representative session evaluation rather than protocol preference.
