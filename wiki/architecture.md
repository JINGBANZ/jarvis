...
|---|---|---|
| **AudioInput** | Capture the mic (`me`) with a plain `AVAudioEngine` tap; stream 24 kHz PCM16 to its Transcriber. No AEC (`VoiceProcessingIO` silenced the mic once the `SCStream` started); speaker bleed is instead removed in the transcript by **Transcript**, so headphones aren't required. | AVFoundation (`AVAudioEngine` tap). |
