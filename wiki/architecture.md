...
- **Transcription has its own provider, model, and language settings.** OpenAI remains the provider
  ...
  Transcribe and GPT Live receive fixed role-aware recording context that asks them to ignore
  music, singing and background noise and not invent speech; GPT Live also requests low
  ...
  speaker may switch within a sentence. The coaching prompt follows the user's conversational
  language unless one language is explicitly selected (or Apple uses a selected locale), in which
  case that language guides replies. This instruction is frozen at Start and shared by per-turn
  and persistent CLI prompts, including later brain-client refreshes. No language detector or
  rejection layer discards incoming speech or model output. Prompt guidance is not a guarantee
  against non-speech hallucinations; language-independent detection needs separate evaluation.
  The macOS 26+ opt-in is Apple `SpeechAnalyzer` with one
