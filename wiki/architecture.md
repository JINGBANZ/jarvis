...
1. The Transcriber emits a **turn-end** event after it finalizes an utterance (GPT-4o and GPT
   Transcribe use tuned server VAD, GPT Live uses local WebRTC VAD plus an explicit acknowledged
...
- **Transcription has its own provider, model, and language settings.** OpenAI remains the provider
  ...
  model-compatible turn contracts: GPT-4o and GPT Transcribe use tuned `server_vad`, while GPT Live
  disables unsupported server turn detection, keeps only bounded local pre-roll while idle, and
  explicitly commits endpoints from classic WebRTC VAD running locally on each post-AEC 48 kHz
  stream. GPT Transcribe and GPT Live receive fixed role-aware recording context; GPT Live also
  requests low transcription delay. Jarvis does not send vocabulary keywords. Automatic is the
  default language profile and sends no language hint. A
