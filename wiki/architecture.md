...
2. Before an automatic attempt, `TranscriptionSettlementGate` checks both provider streams against
   ...
   already produced. Gemini times no utterance at all, so `PendingSpeechWindow` derives its start
   from the capture time of the audio still queued and the local onset of the utterance the server
   is recognizing, capped by the moment that recognition was first reported. Speech the local
   detector missed stays unknown, and so does a whole speaker while one long run of speech is
   recognized as several utterances.
