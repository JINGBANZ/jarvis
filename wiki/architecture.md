...
2. Before an automatic attempt, `TranscriptionSettlementGate` checks both provider streams against
   ...
   detector missed stays unknown, and one long run of speech that the server splits into several
   utterances keeps its first onset until the run ends.
