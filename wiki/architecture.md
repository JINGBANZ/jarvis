...
- **Transcription has its own provider, model, and language settings.** OpenAI remains the provider
  ...
  `server_vad`, while GPT Live disables unsupported server turn detection and explicitly commits
  endpoints from classic WebRTC VAD running locally on each post-AEC 48 kHz stream. Automatic is the
