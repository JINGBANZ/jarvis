...
- **[settings-window.md](./settings-window.md)** — the unified Settings window: one menu item → `SettingsWindow`, which opens on a hub drawn as Jarvis's head with live status per part, computed in Core by `RobotHub`. The brain, ear, eye, and mouth open Brain (the ordered coaching route and reasoning effort), Ear (transcription provider/model/expected-language list or Apple locale), Eye (active window or an entire display for `capture_screen`), and Mouth (both coaching surfaces); the dock opens Connections (API keys and the Codex and Claude Code sign-ins), Tools (prep notes search and its sources), Skills (one switch per coaching skill), Shortcuts, and Activity.
