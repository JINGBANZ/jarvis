...
- **Chose:** Replay the brain's `reasoning` items **verbatim** (ids and any payload untouched, in order, ahead of the replayed `function_call`) on the tool loop's follow-up request, then drop them at commit — they live only inside the turn that produced them, like screenshots.
...
- **Detail:** `Sources/JarvisCore/Coach/OpenAIBrainClient.swift` (verbatim extract/re-emit), `CoachDriver.swift` (thread-then-drop), `Brain.swift` (`ChatMessage.rawItems`).
