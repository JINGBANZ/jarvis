...
- **Detail:** [build-and-run.md → Distribution](./build-and-run.md#distribution--signed-notarized-releases); `scripts/package-app.sh`, `release-please-config.json`.
- **Superseded by:** the 2026-07-21 entry for the *build* half — the macOS sign/notarize/upload moved out of CI to the maintainer's Mac. The release-please half (Release PR, version sync, tag, draft Release) is unchanged.
...
### 2026-07-21 — The distributable macOS build is made locally, not in CI

- **Chose:** `.github/workflows/release.yml` keeps only the release-please job (version bump, CHANGELOG, tag, **draft** Release — all on the free Linux runner); the signed + notarized build is produced on the maintainer's Mac with `scripts/package-app.sh`, then uploaded to the draft Release and published by hand. The `macos-15` publish job (Developer ID keychain import → build/sign → notarize → upload) was removed.
- **Why:** The repo is **private**, so GitHub bills macOS runner minutes at a 10x multiplier — and the account's spending limit blocked the publish job outright, so no notarized asset could ship regardless. The Developer ID certificate already lives on the maintainer's Mac, and `package-app.sh` was written to run in both places (env-var API key for CI, `notarytool` keychain profile locally), so moving the build local costs no new code and removes the only paid, secret-dependent step. A private repo's Release assets aren't publicly downloadable anyway, so the CI publish bought less than it appeared to.
- **Rejected:** (a) Making the repo public for free macOS minutes + a public Releases page — deferred; not willing to open the source yet. (b) Keeping the CI job and raising the GitHub spending limit — recurring per-release cost for a build one Mac already does for free. (c) A self-hosted macOS runner — needs a Mac online on demand; the maintainer's Mac already does this interactively without the runner daemon.
- **Supersedes in part:** the 2026-07-17 distribution entry (the CI *build* half only; the release-please half stands).
- **Detail:** [build-and-run.md → Distribution](./build-and-run.md#distribution--signed-notarized-releases); `.github/workflows/release.yml`, `scripts/package-app.sh`.
