...
- **Transcription = `gpt-4o-transcribe`** over the **GA Realtime API** (`gpt-realtime-2` was not a real ID; `gpt-realtime-whisper` was tried but has no server VAD — needs manual commits — so it was dropped). Connect with `?intent=transcription` (no `OpenAI-Beta` header); `session.type:"transcription"` with config under `session.audio.input` (`format` audio/pcm 24000, `transcription.model`, `turn_detection` server_vad → auto-commit); `AudioInput` targets 24 kHz. Turn-end + transcript come from `…transcription.completed`. The connect URL + payload are extracted into a unit-tested `RealtimeSession` builder. A later multi-agent ultra-review (see below) caught and fixed the model/intent bugs + added production hardening.
