...
- **Transcription = `gpt-realtime-whisper`** (streaming STT) over the **GA Realtime API** (`gpt-realtime-2` was not a real ID). `RealtimeTranscriber` drops the `OpenAI-Beta` header, configures a `session.type:"transcription"` session with config under `session.audio.input` (`format` audio/pcm 24000, `transcription.model`, `turn_detection` server_vad for auto-commit), and `AudioInput` targets 24 kHz. Turn-end + transcript both come from `conversation.item.input_audio_transcription.completed` (no dependence on manual commits). `gpt-4o-transcribe` is a higher-accuracy one-line swap.
