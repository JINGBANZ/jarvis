...
| `ActivitySection` | "Activity" | yes | Embeds the `ActivityViewer` content view (`makeContentView()` / `teardown()`); `fillsTab == true` so the log stretches with the window. Its header shows the selected session's exact directory ID with **Copy ID** and carries **Open report**, enabled only when the dev-side agentic evaluator has written `eval-report.md`. The sole evaluation path is `scripts/eval-session.sh`: its Claude Code / Codex agent receives the complete session directory, reads the full unfiltered `jarvis-activity.jsonl` whenever it needs the user-visible sequence, and correlates it with `brain-traffic.jsonl`, screenshots, and the live source checkout. `EvalReportPage` renders the saved markdown as `eval-report.html`; **Copy as Markdown** hands the raw report to an agent chat. Report opening stays disabled through the live coaching/teardown lifecycle because it presents a browser. |
