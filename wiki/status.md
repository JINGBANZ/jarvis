...
Phase 4 OpenAI provider extraction (the `JarvisBrainProviders` target) are built; the unified
`SessionEvidence` stack, diagnostic migration, Activity projection, offline maintenance
(preemptible compaction and pruning), the remaining adapter moves, and coordinator decomposition
