...
- `Sources/JarvisCore/Providers/` — what a failure at any provider boundary means, classified once and shared by the brain, transcription, and Settings surfaces ([architecture.md → One failure record](./architecture.md#one-failure-record-one-table-per-vendor)): the failure record with its Activity sentence table and exhaustion relabelling, the one redaction path provider text takes before a person can see it, and one classifier per vendor plus the transport table (`ProviderFailure`, `ProviderFailure+Activity`, `ProviderFailure+Exhaustion`, `ProviderMessageRedaction`, `TransportFailureClassifier`, `OpenAI/OpenAIFailureClassifier`, `Gemini/GeminiFailureClassifier`, `LocalAgent/LocalAgentFailureClassifier`, which owns the CLI adapters' error-domain names). Foundation-only under the kernel dependency guard: adapters hand in status codes, JSON, close reasons, and `NSError` domain and code, never `URLSession` types.
