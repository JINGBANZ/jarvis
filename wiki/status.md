...
state. The normal app supplies no benchmark instrumentation: its session contract and the `AppDelegate`
and `SessionComposition` wiring expose no benchmark capability, event construction is skipped, and its direct reconnect path
...
- `Sources/JarvisCore/LiveE2E/` + `Sources/JarvisApp/LiveE2E/`: the Foundation-only scenario model, launch options, and audio timeline, plus the hidden signed-app live e2e mode, whose runner drives one scenario through `SessionComposition` over a fixture audio source fed by synthesized speech (`LiveE2EOptions`, `LiveE2EScenario`, `AudioTimeline`, `LiveE2EAppDelegate`, `LiveE2ERunner`, `FixtureAudioSource`, `FixtureSpeech`; contract in [live-e2e-tests.md](./live-e2e-tests.md)).
- `Tests/JarvisLiveTests/` + `scripts/run-live-tests.sh`: the live e2e checker and its command. The Gate compiles the target but never runs it. It holds the scenario files, the fixtures, the launcher's preflight, launch, and screen handshake, and one test per scenario asserting each case on the session folder (`LiveE2ETests`, `LiveE2ELauncher`).
