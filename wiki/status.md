...
Run the Gemini live smoke with a real Gemini key: confirm selecting Gemini and pressing Start with no
Gemini key saved refuses the Start and names the missing credential; with the key saved, confirm Start
reaches the ready state and microphone speech appears in Activity as `heard` lines, then confirm
system-audio speech appears from the other side; confirm `jarvis-debug.log` shows the Gemini endpoint
without ever logging its `key=` query (`grep -c "key=" .jarvis/*/jarvis-debug.log` prints `0`); finally
switch back to OpenAI and start again to confirm the primary path is unregressed. Not yet run.
