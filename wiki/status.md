...
only in explicit Coding mode. The header’s protected mode dropdown and Brain Settings both start a
fresh session on a different selection, clearing conversation and overlay content after successful
preflight; see [settings-window.md](./settings-window.md#overlay-appearance).
...
- `Sources/JarvisOverlay/` — the capture-invisible `NSPanel` surfaces: `OverlayCaptionPanel` (transient), `OverlayBoxPanel` (persistent), `NSPanel+CaptureExclusion`; plus the box's own chrome — `OverlayBoxHeaderView` and `OverlayFormatPickerView` (the clickable session format and protected mode list) and `OverlayBoxHeaderButton` (collapse, the name, clear), `OverlayBoxChrome` (header geometry derived from the box's height), and `OverlayBoxResizeAffordanceView` (the drawn edge affordance, which also owns the resize drag because macOS refuses an inactive app a resize cursor).
