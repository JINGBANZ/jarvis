...
| **Overlay surfaces renamed + independently switchable** — the transient on-screen tip is the **Overlay Caption** (`OverlayCaptionPanel`) and the persistent history is the **Overlay Box** (`OverlayBoxPanel`). Each has an on/off checkbox + a one-line description in the Overlay settings tab; defaults are caption **off**, box **on**. The menu's Show/Hide Responses item is removed — visibility now lives only in Settings. Repo-wide rename for consistency: `OverlayAppearance` props/keys (`overlayCaption.*`/`overlayBox.*`), `Config` defaults, and the `OverlayCaptionApplying`/`OverlayBoxApplying` protocols (2026-06-21) | [settings-window.md](./settings-window.md) |
