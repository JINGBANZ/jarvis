...
let package = Package(
    ...
    targets: [
        ...
        .executableTarget(
            ...
            // Already-compiled Core ML model, regenerated by scripts/build-vad.sh and committed like
            // the AEC archive. `.copy` keeps the .mlmodelc directory intact; `.process` would flatten
            // it. Reached through `Bundle.module`, so it resolves for `swift build`, the benchmark
            // harness, and the assembled app alike.
