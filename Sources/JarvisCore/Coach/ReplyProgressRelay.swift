...
final class ReplyProgressRelay: @unchecked Sendable {
    ...
    init(overlay: OverlayRendering, config: Config, detailEnabled: Bool) {
    ...
    func beginRequest() {
        ...
        let consumer = Task { @MainActor in
            ...
            for await snapshot in stream {
                ...
                overlay.showReplyProgress(snapshot, perLineSeconds: snapshot.closedLines.map {
    ...
    private func receive(_ delta: ToolCallDelta) -> [String] {
        guard delta.index == 0, delta.name == speakToolName,
              let scanned = SpeakArgumentsScanner.progress(in: delta.arguments) else { return [] }
        // A session without the box never promised a detail, so one the model writes anyway stays out.
        let snapshot = detailEnabled ? scanned : BrainReplyProgress(
            closedLines: scanned.closedLines, openLine: scanned.openLine,
            linesComplete: scanned.linesComplete, detailMarkdown: nil)
