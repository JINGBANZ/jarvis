...
// @unchecked: `summary` is the only cross-queue mutable state and every access is guarded by
// `lock`. `NWPathMonitor` calls back on `queue`, which actor isolation cannot express.
