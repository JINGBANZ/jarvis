...
@Suite struct GeminiFailureClassifierTests {
    ...
    /// Google reports a bad key and an unsupported region both as HTTP 400, told apart by status.
    ...
    /// Google closes with 1008 for a bad key, a retired model, or a bad request; only the reason
    /// differs. Unrecognized reasons stay .configuration so a user is never sent to rotate a good
    /// key.
