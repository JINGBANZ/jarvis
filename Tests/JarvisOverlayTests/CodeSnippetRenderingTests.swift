...
@Suite struct CodeSnippetRenderingTests {
    ...
    @MainActor @Test func hintsPreserveCodeAndPreviewCannotReplaceLiveDelivery() throws {
        ...
        box.showAppearancePreview(false)
        #expect(box.currentCodeSnippet == snippet)
        // A text-only speak delivers nil before its text; keep the code without reporting it anew.
        #expect(BroadcastOverlay([box]).deliverCodeSnippet(nil) == nil)
        _ = box.deliver(["Explain the tradeoff"], perLineSeconds: [1], diagram: nil, explanation: nil)
        #expect(box.currentCodeSnippet == snippet)
        box.clickCollapseButton()
        #expect(box.deliverCodeSnippet(nil) == nil)
        box.clickCollapseButton()
        #expect(box.currentCodeSnippet == snippet)
        let replacement = try #require(CodeSnippet(language: "swift", placement: "Next step", code: "  return result"))
        #expect(box.deliverCodeSnippet(replacement) == replacement)
        #expect(box.currentCodeSnippet == replacement)
        box.setSessionLive(false)
        #expect(box.currentCodeSnippet == nil)
        box.setSessionLive(true)
