...
- **Keep human activity separate from agent diagnostics.** `ActivityLog` is the human-facing coaching
  ...
  success or failure, `speak`, and `stay_silent`), and fixed typed notices for every runtime failure
  that stops or degrades coaching belong there. Lifecycle, transport, retry, raw error, timing, and
  diagnosis details go through `jlog` to `jarvis-debug.log`. Never mirror `jlog` into `ActivityLog`;
  record the corresponding typed activity event explicitly.
